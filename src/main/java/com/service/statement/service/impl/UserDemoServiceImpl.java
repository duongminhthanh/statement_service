...
import com.service.statement.mapper.UserDemoMapper;
...
public class UserDemoServiceImpl implements IUserDemoService {
    ...
    @Override
    public BaseResponse createUserDemo(UserDemo user) throws MessagingException {
        ...
        Integer countUserCode = userDemoMapper.getCountByUserCode(user.getCode());
        if (countUserCode > 0) {
            return responseStatus("-2", "UserCode already in use");
        }

        Integer countPhoneNo = userDemoMapper.getCountByPhoneNo(user.getPhoneNo());
        if (countPhoneNo > 0) {
            return responseStatus("-3", "PhoneNo already in use");
        }

        Integer countByEmail = userDemoMapper.getCountByEmail(email);
        if (countByEmail > 0) {
        ...
        Integer countByFullName = userDemoMapper.getCountByFullName(name);
        ...
        UserDemo userRes = userDemoMapper.createUserDemo(user);
    ...
    @Override
    public BaseResponse getUserDemoPaging(UserDemoRequest request) {
        List<UserDemo> list = userDemoMapper.search(request);
        int totalRecords = userDemoMapper.count(request);
        BaseResponse result = new BaseListResponse(list, totalRecords, request.getLimit());
        return result;
    }

    @Override
    public BaseResponse updateUserDemo(UserDemo request) {
        BaseResponse response = new BaseResponse();

        UserDemo user = userDemoMapper.updateUserDemo(request);
        Integer countUserCode = userDemoMapper.getCountByUserCode(user.getCode());
        if (countUserCode > 0) {
            return responseStatus("-1", "UserCode already in use");
        }
        if (user != null) {
    ...
    @Override
    public BaseResponse deleteUserDemo(UserDemo request) {
        BaseResponse response = new BaseResponse();

        int userDeleted = userDemoMapper.deleteUserDemo(request.getId());
        if (userDeleted > 0) {
            response.setErrorCode("0");
    ...
    @Override
    public UserDemo validateUser(String email, String password) throws EtAuthException {
        if (email != null) email = email.toLowerCase();
        try {
            UserDemo user = userDemoMapper.findByEmailAndPassword(email.toUpperCase());
    ...
    @Override
    public BaseResponse importFile(MultipartFile file, String creater) {
        if (file != null) {
            try (@SuppressWarnings("deprecation")
                ...
                while (rowIterator.hasNext()) {
                    ...
                    Integer countUserCode=userDemoMapper.getCountByUserCode(code.getStringCellValue());
                    if (countUserCode > 0) {
                        return responseStatus("-2", "UserCode already in use");
                    }
                    users.add(user);
                    userDemoMapper.importExcel(users);
                    users.clear();
                }
    ...
    @Override
    public File exportUser(UserDemoRequest request) {
        File file = null;
